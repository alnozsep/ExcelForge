...
def build_extraction_prompt(
    ...
    if template_text:
        ...
        prompt += "判定ルール:\n"
        prompt += "1. ラベル（例: 'ご請求金額'）の右隣や、すぐ下の '(empty)' となっているセルが入力候補です。\n"
        prompt += "2. 結合セル（Merged Cells）の場合、その範囲内のいずれかの番地を指定してください。\n"
        prompt += "3. 明細行（テーブル）の場合、ヘッダ行の下の '(empty)' セルに順番にデータを並べてください。\n"
        prompt += "4. 請求日や宛名など、ヘッダ情報の位置も正確に特定してください。\n"
        prompt += "出力は以下のJSON形式とします。キーは 'シート名:セル番地' としてください:\n"
        prompt += '{\n  "SheetName:A1": "抽出値",\n  "SheetName:B2": 12345\n}\n'
    elif mapping_config:
