...
def main():
    ...
    with st.form("upload_form"):
        ...
        st.info("⚠ アップロードされたファイルは処理後即座に削除されます。サーバーには保存されません。")

        submit_btn = st.form_submit_button("▶ 変換を開始する", type="primary", use_container_width=True)
    ...
    if submit_btn:
        ...
        with st.status("処理中です...", expanded=True) as status:
            ...
            try:
                ...
                files = {
                    "source_file": (source_file.name, source_file.getvalue(), "application/octet-stream"),
                ...
                if response.status_code == 200:
                    ...
                    try:
                        ...
                        st.error(f"エラー: {err_data.get('detail', '不明なエラー')} (Code: {err_data.get('error_code', '')})")
                    except Exception:
                        st.error(f"サーバーエラーが発生しました (HTTP {response.status_code})")
            except httpx.ReadTimeout:
                status.update(label="タイムアウト", state="error", expanded=False)
                st.error("処理がタイムアウトしました。サーバーの負荷が高いか、データが大きすぎます。")
