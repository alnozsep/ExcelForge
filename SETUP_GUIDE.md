...
- `Artifact Registry API` (Dockerイメージの保存用)
...
### 2-6. Artifact Registry リポジトリの作成
ビルドしたDockerイメージを保存するための場所（リポジトリ）を作成します。

1. Cloud Consoleのメニューから `[Artifact Registry] > [リポジトリ]` に移動します。
2. 画面上部の `[＋リポジトリを作成]` をクリックします。
3. 以下の通り入力します：
   - 名前: `excelforge-repo`
   - 形式: `Docker`
   - ロケーションタイプ: `リージョン`
...
> - [ ] Artifact Registryに `excelforge-repo` という名前のリポジトリを作成した
