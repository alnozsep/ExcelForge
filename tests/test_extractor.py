...
async def test_successful_extraction(mock_vertex_model):
    ...
    mock_vertex_model.generate_content_async = AsyncMock(return_value=MockResponse('{"会社名": "株式会社テスト"}'))
...
async def test_extraction_with_mapping_config(mock_vertex_model):
    ...
    mock_vertex_model.generate_content_async = AsyncMock(return_value=MockResponse('{"代表者名": "山田太郎"}'))
...
async def test_retry_on_json_parse_failure(mock_vertex_model):
    ...
    mock_vertex_model.generate_content_async = AsyncMock(side_effect=[
...
async def test_max_retry_exceeded_raises_error(mock_vertex_model):
    ...
    mock_vertex_model.generate_content_async = AsyncMock(return_value=MockResponse('不正なJSON文字列'))
    
    with pytest.raises(AppException) as excinfo:
        await extract_data("テキスト")
    
    assert excinfo.value.error_code == ErrorCode.EXTRACTION_FAILED
    assert mock_vertex_model.generate_content_async.call_count == 3  # 初回 + リトライ2回

@pytest.mark.asyncio
async def test_timeout_raises_error(mock_vertex_model):
    """タイムアウト時にTimeoutErrorが送出されること"""
    mock_vertex_model.generate_content_async = AsyncMock(side_effect=asyncio.TimeoutError())
