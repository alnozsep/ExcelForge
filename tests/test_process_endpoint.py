...
from app.core.masking import MaskingResult
...
def mock_dependencies():
    with patch("app.api.endpoints.process.extract_data", new_callable=AsyncMock) as mock_extract, \
...
def test_successful_processing(auth_token, mock_dependencies, sample_pdf_bytes, sample_xlsx_bytes):
    """正常な入力でExcelが返却されること"""
    mock_extract, mock_write = mock_dependencies
    
    files = {
        "source_file": ("test.pdf", sample_pdf_bytes, "application/pdf"),
        "template_file": ("template.xlsx", sample_xlsx_bytes, "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
    }
    data = {
        "token": auth_token
    }
...
def test_invalid_file_type_returns_400(auth_token, sample_xlsx_bytes):
    """非対応ファイルで400が返却されること"""
    files = {
        "source_file": ("test.txt", b"dummy text file content just to be sure it is not pdf", "text/plain"),
        "template_file": ("template.xlsx", sample_xlsx_bytes, "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
    }
...
def test_receipt_header_present(auth_token, mock_dependencies, sample_pdf_bytes, sample_xlsx_bytes):
